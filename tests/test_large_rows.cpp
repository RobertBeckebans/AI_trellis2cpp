...
bool run_op(ggml_backend_t backend, int op, int64_t L,
            const std::vector<float> & xdata, const std::vector<int32_t> & idata,
            const std::vector<float> & mdata, const std::vector<float> & wdata,
    ...
    if (idx)  ggml_backend_tensor_set(idx,  idata.data(), 0, ggml_nbytes(idx));
    if (mask) ggml_backend_tensor_set(mask, mdata.data(), 0, ggml_nbytes(mask));
...
int main(int argc, char ** argv) {
    ...
    if (L <= 0) { std::fprintf(stderr, "bad L\n"); return 1; }
    ...
    for (auto & v : xdata) v = lcg(s);
    std::vector<int32_t> idata((size_t) L);
    for (int64_t i = 0; i < L; ++i) idata[(size_t) i] = (int32_t) ((i * 7919 + 13) % L);  // scattered
    std::vector<float> mdata((size_t) L);
    for (int64_t i = 0; i < L; ++i) mdata[(size_t) i] = (i % 5) ? 1.0f : 0.0f;            // like the neighbor mask
    std::vector<float> wdata((size_t) C * C);
    for (auto & v : wdata) v = lcg(s);

    int failures = 0, skipped = 0;
    for (int op = 0; op < OP_COUNT; ++op) {
        std::vector<float> g, c;
        if (!run_op(gpu, op, L, xdata, idata, mdata, wdata, g)) { ++skipped; continue; }
        if (!run_op(cpu, op, L, xdata, idata, mdata, wdata, c)) { ++skipped; continue; }
        if (g.size() != c.size()) {
