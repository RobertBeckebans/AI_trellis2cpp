...
rem
rem   Builds into build-shared/ so it does NOT overwrite the GPU DLLs in
rem   build-shared/ — both can coexist, pick one with the server's -lib flag.
rem
...
echo.
echo Fertig: build-shared\bin\Release\trellis2.dll  (+ ggml.dll, ggml-base.dll, ggml-cpu.dll)
echo.
echo Starten aus server\:
echo   trellis2-server.exe -lib ..\build-shared\bin\Release\trellis2.dll
pause
endlocal
